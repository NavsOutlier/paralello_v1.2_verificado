...
export function useCampaignInsights({
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            ...
            try {
                ...
                // REAL SUPABASE DATA
                let queryTable = '';
                if (level === 'campaigns') queryTable = 'v_meta_campaign_insights';
                else if (level === 'adsets') queryTable = 'v_meta_adset_insights';
                else queryTable = 'v_meta_ad_insights';
                ...
                // Add Hierarchical Filters Based on View Level
                ...
                // Sorting handled by DB index on dates/views ideally, keeping simple
    ...
    // not split by day natively in the table list view, unless we are drawing the chart.
    // For the main table, it expects 1 row = 1 entity totalized. 
    const data = useMemo<AggregatedInsight[]>(() => {
        ...
        // O backend View retorna diário para permitir gráficos de linha (LineChart). 
        // Para a lista Table(Dashboard base), o React precisa compilar os dias selecionados (ex: 7 dias = 1 card)
        ...
        return Object.entries(grouped).map(([id, rows]: [string, any[]]) => {
            ...
            return {
                ...
                // Somatória apenas do range visualizado, já otimizado pelas views que reduziram o dataset brutalmente
                ...
                link_clicks: rows.reduce((s: number, r: any) => s + (Number(r.link_clicks) || 0), 0),
                spend: rows.reduce((s: number, r: any) => s + (Number(r.spend) || 0), 0),
                leads: rows.reduce((s: number, r: any) => s + (Number(r.leads) || 0), 0),
