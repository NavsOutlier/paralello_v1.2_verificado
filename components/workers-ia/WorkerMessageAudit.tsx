                ...
                <div className="p-6 border-b border-cyan-500/10 bg-slate-800/30 flex items-center justify-between">
                    ...
                    <div className="flex flex-col items-end gap-1">
                        ...
                        <div className="mt-1 flex items-center gap-2">
                            <span className="text-[9px] px-1.5 py-0.5 bg-cyan-500/10 text-cyan-400 rounded-md border border-cyan-500/20 uppercase font-bold tracking-wider">
                                ...
                                return (
                                    <div key={msg.id} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                                        <div className={`max-w-[80%] rounded-2xl p-4 shadow-xl relative ${msg.role === 'user'
                                            ...
                                            : msg.role === 'support'
                                                ? 'bg-gradient-to-br from-slate-700 to-slate-800 text-slate-200 border border-cyan-500/30 rounded-tl-none'
                                                : isHandoff
                    ...
                    <div className="w-80 bg-slate-900 border-l border-cyan-500/10 p-6 flex flex-col gap-6">
                        ...
                        <div className="space-y-4">
                            <div>
                                ...
                                <div className="grid grid-cols-2 gap-2">
                                    {['price', 'competitor', 'no_fit', 'scheduled'].map(id => (
                                        <button key={id} onClick={() => updateAuditFields({ loss_reason: id })} className={`px-3 py-2 rounded-xl text-[10px] font-bold border transition-all ${selectedConv.loss_reason === id ? 'bg-cyan-500/20 border-cyan-500/50 text-white' : 'bg-slate-800/40 border-slate-700/50 text-slate-500 hover:border-slate-600'}`}>
                            ...
                            <textarea
                                ...
                                className="w-full h-32 bg-slate-800/40 border border-slate-700/50 rounded-xl p-3 text-xs text-slate-300 placeholder-slate-600 focus:ring-2 focus:ring-cyan-500/50 outline-none transition-all resize-none"
                            />
                            <button onClick={saveAuditData} disabled={loadingSave} className="w-full py-3 bg-gradient-to-r from-cyan-600 to-indigo-700 rounded-xl text-xs font-black text-white hover:from-cyan-500 hover:to-indigo-600 transition-all flex items-center justify-center gap-2 uppercase tracking-widest">
            ...
            <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4">
                ...
                <div className="flex flex-wrap gap-3 w-full lg:w-auto">
                    <div className="relative flex-1 lg:w-64">
                        ...
                        <input type="text" placeholder="Buscar..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full pl-10 pr-4 py-2 bg-slate-800/50 border border-cyan-500/10 rounded-xl text-sm text-white focus:ring-2 focus:ring-cyan-500/50 outline-none" />
                    </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {loading ? (
                    ...
                    filteredConversations.map((conv) => (
                        <button key={conv.id} onClick={() => setSelectedConv(conv)} className="p-5 bg-slate-900/40 hover:bg-slate-800/60 border border-cyan-500/10 hover:border-cyan-500/50 rounded-2xl text-left transition-all group relative overflow-hidden shadow-lg">
                            <div className="flex justify-between items-start mb-4">
                                <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-cyan-500/20 to-fuchsia-500/20 border border-cyan-500/20 flex items-center justify-center text-cyan-400"><User className="w-5 h-5" /></div>
                                {(() => {
                            ...
                            <div className="flex items-center justify-between text-[9px] text-slate-500 font-bold uppercase tracking-widest pt-3 border-t border-white/5">
                                ...
                                <span className="flex items-center gap-1 group-hover:text-cyan-400 transition-colors">Ver Chat <ChevronRight className="w-3 h-3" /></span>
                            </div>
                        </button>
                    ))
                )}
