...
import {
    ...
    Smartphone, Loader2, RefreshCw, QrCode, ArrowRight, Check
} from 'lucide-react';
...
export const WorkerConfig: React.FC<WorkerConfigProps> = ({
    ...
    // Internal state for the Agent being created/edited
    // Initialize with props, but update as we save steps
    ...
    // Instance name uses client ID/Name but theoretically enables multiple agents per client if named differently
    ...
    useEffect(() => {
        ...
        if (connectingWs && isWaitingQr && !isConnected) {
            interval = setInterval(() => {
                // Ensure we have an agent ID before polling
                if (currentAgent?.id) {
    ...
    const handleConnectWhatsApp = async () => {
        if (!currentAgent?.id) {
            alert('Erro: Agente não identificado. Salve as configurações anteriores primeiro.');
            return;
        }
    ...
    const handleSaveStep = async () => {
        ...
        try {
            ...
            if (currentAgent?.id) {
                ...
                // NEW: Auto-create instance with 'disconnected' status when agent is created
                if (!myInstance) {
            ...
            if (currentStep === 'brain') {
                ...
                // Only notify parent when reaching final step (so modal stays open during wizard)
                    ...
                    {currentStep === 'identity' && (
                        <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
                            <div className="space-y-4">
                                ...
                                <div className="flex items-center justify-between p-4 bg-slate-800/50 rounded-xl border border-slate-700/50 mt-4">
                                    <div>
                                        <p className="text-sm font-medium text-white">Status Inicial</p>
                                        <p className="text-xs text-slate-500">
                                            Worker inicia {formData.is_active ? 'Ativo' : 'Pausado'}
                                        </p>
                                    </div>
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="space-y-2">
                                    ...
                                    <textarea
                                        ...
                                        className="w-full h-80 px-4 py-4 bg-slate-800 border border-slate-700 rounded-xl text-white placeholder-slate-600 focus:ring-2 focus:ring-violet-500 focus:outline-none font-mono text-sm leading-relaxed resize-none custom-scrollbar"
                                    />
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
                            <div className="text-center space-y-4">
                                ...
                                <p className="text-slate-400 max-w-md mx-auto">
                                    O Worker já foi criado com sucesso. Agora, conecte o número para ativá-lo.
                                    Você pode fazer isso agora ou mais tarde.
                                </p>

                                <div className="bg-slate-800/50 p-6 rounded-2xl border border-slate-700 mt-8 max-w-sm mx-auto">
                                    {isConnected ? (
                                            ...
                                            {!connectingWs && !isWaitingQr ? (
                                                <div className="flex flex-col items-center">
                                                    ...
                                                    <button
                                                        ...
                                                        className="px-6 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl font-bold transition-all shadow-lg shadow-emerald-500/20 flex items-center gap-2"
                                                    >
                                                ...
                                                <div className="flex flex-col items-center justify-center py-4">
                                                    {myInstance?.qrCode ? (
                                                        <div className="space-y-4">
                                                            <div className="bg-white p-3 rounded-2xl border-4 border-emerald-500/30 shadow-2xl">
                                                                <img
