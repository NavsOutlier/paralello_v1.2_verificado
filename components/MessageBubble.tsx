...
export const MessageBubble: React.FC<MessageBubbleProps> = ({ 
  ...
  return (
    ...
    >
      <div className={`relative max-w-[85%] rounded-lg p-3 shadow-sm text-sm ${
        msg.isInternal 
        ...
        {/* Sender Name */}
        {!isMe && (
           <div className={`text-[10px] font-bold mb-1 ${msg.isInternal ? 'text-yellow-700' : 'text-indigo-600'}`}>
             {senderName} {msg.isInternal && '(INTERNO)'}
           </div>
        )}
        ...
        {msg.linkedMessageId && (
           <div 
             ...
             className={`mt-2 p-2 rounded text-[10px] cursor-pointer transition-colors flex items-center gap-2 ${
               msg.isInternal ? 'bg-yellow-100 hover:bg-yellow-200 text-yellow-800' : 'bg-indigo-700 hover:bg-indigo-800 text-indigo-100'
             }`}
           >
