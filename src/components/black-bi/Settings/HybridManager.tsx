...
import { Bot, User as UserIcon, Settings2, Smartphone, ShieldCheck, Zap, Target, Tag, Sparkles, Plus, X, Flame, Shuffle, Trash2, Layout, MessageSquare, Phone, Filter, BookOpen, Layers, FileText, Mic, PlayCircle, Copy, Calendar, CheckCircle, Loader2 } from 'lucide-react';
import { supabase } from '../../../../lib/supabase';

// Mock toast to avoid dependency issues if not installed
const toast = {
...
export const HybridManager: React.FC<HybridManagerProps> = ({ clientId }) => {
    ...
    const [sellerTrainings, setSellerTrainings] = useState<any[]>([]);
    const [isAddingTraining, setIsAddingTraining] = useState(false);
    const [newTrainingTitle, setNewTrainingTitle] = useState('');
    const [newTrainingDesc, setNewTrainingDesc] = useState('');
    const [newTrainingCategory, setNewTrainingCategory] = useState('Geral');
    ...
    const fetchData = useCallback(async () => {
        ...
        try {
            ...
            if (aiData) {
                setAiConfig(aiData);
                setIcpDefinition(aiData.icp_definition || '');
                setHotLeadThreshold(aiData.hot_lead_threshold || 80);
                if (aiData.scoring_weights) setCriteriaScores(aiData.scoring_weights);
            }
            ...
            if (triggers) {
                setHandoffTriggers(triggers.map(t => ({
                    id: t.id,
                    name: t.keyword,
                    type: t.trigger_type,
                    satisfactionLimit: t.satisfaction_threshold
                })));
            }
            ...
            if (teamData) setSellers(teamData);
            ...
            if (instance) setInstanceData(instance);
        ...
        try {
            const { error } = await supabase
                ...
                .update({
                    ...
                    scoring_weights: criteriaScores,
    ...
    const fetchTrainings = async (sellerId: string) => {
        try {
            const { data } = await supabase
                .from('sdr_training_history')
                .select('*')
                .eq('team_member_id', sellerId)
                .order('training_date', { ascending: false });

            if (data) setSellerTrainings(data);
        } catch (error) {
    ...
    const handleSaveTrigger = async () => {
        if (!triggerName.trim()) {
            toast.error('Nome do gatilho é obrigatório');
            return;
        }
        setSaving(true);
        try {
            const payload = {
                ...
                satisfaction_threshold: satisfactionLimit,
                is_active: true,
                target_stage: 'interessado',
                action: 'notify_human'
            };

            let res;
            if (isEditingTrigger && isEditingTrigger !== 'new') {
                res = await supabase
                    .from('handoff_triggers')
                    .update(payload)
                    .eq('id', isEditingTrigger);
            } else {
                res = await supabase
                    .from('handoff_triggers')
                    .insert([payload]);
    ...
    const handleRegisterTraining = async (title: string, desc: string, category: string) => {
        if (!selectedSeller) return;
        try {
            const { error } = await supabase
                .from('sdr_training_history')
                .insert([{
                    team_member_id: selectedSeller.id,
                    organization_id: selectedSeller.organization_id,
                    title,
                    description: desc,
                    category,
            ...
            fetchTrainings(selectedSeller.id);
        } catch (error) {
    ...
    if (loading) {
        return (
            <div className="flex-1 flex flex-col items-center justify-center bg-slate-900/30 p-12">
                <Loader2 className="w-12 h-12 text-emerald-400 animate-spin mb-4" />
                <p className="text-slate-400 font-bold uppercase tracking-widest text-xs">Carregando Sentinela...</p>
            </div>
        );
    }
            ...
            <div className="flex-1 overflow-y-auto p-6 custom-scrollbar">
                    ...
                    <div className="space-y-8 max-w-5xl mx-auto animate-in fade-in slide-in-from-right-4 duration-500">
                        ...
                        <div className="flex items-center justify-between pb-6 border-b border-white/5">
                            ...
                            <div className="flex items-center gap-3">
                                ...
                                <div className={`flex items-center gap-2 px-4 py-2 rounded-full border ${aiConfig?.is_active ? 'bg-emerald-500/10 border-emerald-500/20' : 'bg-rose-500/10 border-rose-500/20'}`}>
                                    <div className={`w-2 h-2 rounded-full animate-pulse shadow-[0_0_8px_rgba(16,185,129,0.5)] ${aiConfig?.is_active ? 'bg-emerald-500' : 'bg-rose-500'}`} />
                                    <span className={`text-[9px] font-black uppercase tracking-widest ${aiConfig?.is_active ? 'text-emerald-400' : 'text-rose-400'}`}>
                                        {aiConfig?.is_active ? 'Sentinela Ativo' : 'Sentinela Inativo'}
                                    </span>
                                </div>
                            </div>
                        </div>
                                        ...
                                        <div className="bg-slate-900/40 border border-white/5 p-6 rounded-[24px]">
                                            ...
                                            <div className="space-y-4">
                                                ...
                                                <div className="space-y-2">
                                                    ...
                                                    <select
                                                        value={aiConfig?.role || 'sdr'}
                                                        onChange={(e) => setAiConfig({ ...aiConfig, role: e.target.value })}
                                                        className="w-full bg-black/40 border border-white/10 rounded-xl px-4 py-3 text-sm text-white font-bold outline-none focus:border-emerald-500 transition-all appearance-none">
                                    ...
                                    <div className="space-y-6">
                                        <div className="bg-emerald-500/5 border border-emerald-500/10 p-6 rounded-[24px] flex items-center justify-between">
                                            ...
                                            <div
                                                ...
                                                className={`w-14 h-7 rounded-full border relative flex items-center px-1 cursor-pointer transition-all ${aiConfig?.is_active ? 'bg-emerald-500/20 border-emerald-500/40' : 'bg-slate-700/20 border-white/10'}`}>
                                                <div className={`w-5 h-5 bg-emerald-500 rounded-full transition-all shadow-lg ${aiConfig?.is_active ? 'ml-auto shadow-emerald-500/50' : 'ml-0 bg-slate-500'}`} />
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                    ...
                                    <div className="grid grid-cols-2 gap-8">
                                        <div className="bg-slate-900/40 border border-white/5 p-6 rounded-[24px]">
                                            <div className="flex items-center justify-between mb-4">
                                                ...
                                                <span className="text-xs font-black text-emerald-400">{aiConfig?.temperature || 0.7}</span>
                                            </div>
                        ...
                        <div className="pt-8 flex justify-end">
                            <button
                                ...
                                disabled={saving}
                                className="flex items-center gap-3 px-12 py-5 bg-emerald-500 text-black text-xs font-black uppercase tracking-[0.2em] rounded-2xl hover:bg-emerald-400 transition-all shadow-xl shadow-emerald-500/20 hover:scale-[1.02] active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed">
                                {saving && <Loader2 className="w-4 h-4 animate-spin" />}
                                {saving ? 'Salvando...' : 'Atualizar Sentinela'}
                            </button>
                        </div>
                    </div>
                )}
                                    ...
                                    {sellers.length === 0 && (
                                        <tr>
                                            <td colSpan={4} className="p-8 text-center text-slate-500 italic">
                                                Nenhum vendedor encontrado nesta organização.
                                            </td>
                                        </tr>
                                    )}
                                    {sellers.map((seller) => (
                                        <tr key={seller.id} className="border-b border-slate-800/50 hover:bg-slate-800/30 transition-colors">
                                            <td className="p-4 font-bold flex items-center gap-3">
                                                <div className="w-8 h-8 rounded-full bg-orange-500/20 text-orange-400 flex items-center justify-center uppercase">
                                                    {seller.profiles?.name?.substring(0, 2) || 'S'}
                                                </div>
                                                {seller.profiles?.name || 'Vendedor s/ nome'}
                                            </td>
                                            <td className="p-4 font-mono text-slate-400">{seller.profiles?.age || '--'} anos</td>
                                            <td className="p-4">
                                                <div className="flex items-center gap-2">
                                                    <span className="text-[10px] text-slate-500 font-bold uppercase tracking-widest italic">
                                                        {seller.job_title || 'SDR / Vendas'}
                                                    </span>
                                                </div>
                                            </td>
                ...
                {activeSection === 'instance' && (
                    <div className="space-y-8 animate-in fade-in slide-in-from-right-4 duration-500 max-w-4xl mx-auto">
                        ...
                        <div className="bg-slate-900/40 border border-white/5 rounded-[40px] p-10 mt-8">
                            {!instanceData ? (
                                <div className="bg-black/40 border border-white/5 border-dashed rounded-[32px] p-16 flex flex-col items-center justify-center text-center">
                                    ...
                                    <h3 className="text-lg font-black text-white uppercase tracking-tight italic mb-2">Nenhuma Conexão Encontrada</h3>
                                    <p className="text-sm text-slate-500 font-medium mb-8 max-w-sm">Conecte um WhatsApp para que o Sentinela possa realizar atendimentos automáticos.</p>
                                    <button className="px-8 py-4 bg-cyan-600 hover:bg-cyan-500 text-white text-[10px] font-black uppercase tracking-[0.2em] rounded-xl transition-all shadow-lg shadow-cyan-600/20 active:scale-95">
                                        CONECTAR WHATSAPP
                                    </button>
                                </div>
                            ) : (
                                <div className="bg-black/40 border border-white/5 rounded-[32px] p-8 max-w-2xl mx-auto relative overflow-hidden group">
                                    ...
                                    <div className="flex items-start justify-between relative z-10">
                                        <div className="flex items-center gap-6">
                                            ...
                                            <div>
                                                <div className="flex items-center gap-3">
                                                    <h4 className="text-xl font-black text-white uppercase tracking-tight italic">{instanceData.name || 'Comercial Principal'}</h4>
                                                    <span className={`px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-widest animate-pulse border ${instanceData.status === 'open' ? 'bg-emerald-500/10 border-emerald-500/20 text-emerald-400' : 'bg-rose-500/10 border-rose-500/20 text-rose-400'}`}>
                                                        {instanceData.status === 'open' ? 'Online' : 'Desconectado'}
                                                    </span>
                                                </div>
                                                <p className="text-sm text-slate-500 font-bold mt-1 tracking-wider">{instanceData.phone || 'Sem número cadastrado'}</p>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div className="mt-10 space-y-px bg-white/5 rounded-2xl overflow-hidden border border-white/5">
                                        ...
                                        <div className="flex items-center justify-between p-5 bg-black/20">
                                            <span className="text-[10px] font-black text-slate-500 uppercase tracking-[0.2em]">Key da Instância:</span>
                                            <span className="text-[9px] font-mono text-slate-500 uppercase tracking-widest">{instanceData.instance_key?.substring(0, 15)}...</span>
                                        </div>
                                    </div>
                ...
                {activeSection === 'score' && (
                    <div className="space-y-8 max-w-5xl mx-auto animate-in fade-in slide-in-from-bottom-4 duration-500">
                        ...
                        {/* Save Footer for Score */}
                        <div className="pt-8 flex justify-end border-t border-white/5">
                            <button
                                onClick={handleSaveAI}
                                disabled={saving}
                                className="flex items-center gap-3 px-12 py-5 bg-fuchsia-600 text-white text-xs font-black uppercase tracking-[0.2em] rounded-2xl hover:bg-fuchsia-500 transition-all shadow-xl shadow-fuchsia-600/20 hover:scale-[1.02] active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed">
                                {saving && <Loader2 className="w-4 h-4 animate-spin" />}
                                {saving ? 'Salvando...' : 'Salvar Estratégia de Qualificação'}
                            </button>
                        </div>
                    </div>
                )}
                                            ...
                                            <div className="flex items-center justify-between mb-8">
                                                ...
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    <button
                                                        ...
                                                        disabled={saving}
                                                        className="flex items-center gap-2 px-6 py-2.5 bg-orange-500 hover:bg-orange-400 text-black text-[10px] font-black uppercase tracking-widest rounded-xl transition-all disabled:opacity-50"
                                                    >
                            ...
                            <div className="flex items-center justify-between p-6 border-b border-white/5">
                                <div className="flex items-center gap-4">
                                    <div className="w-12 h-12 rounded-full bg-orange-500/20 text-orange-400 flex items-center justify-center font-black text-lg">
                                        {selectedSeller.profiles?.name?.substring(0, 2) || 'S'}
                                    </div>
                                    <div>
                                        <h3 className="text-xl font-black text-white">{selectedSeller.profiles?.name || 'Vendedor'}</h3>
                                        <p className="text-sm text-slate-400">{selectedSeller.profiles?.age || '--'} anos</p>
                                    </div>
                                </div>
                            ...
                            <div className="p-6 flex-grow overflow-y-auto space-y-8">
                                <div>
                                    <div className="flex items-center justify-between mb-4">
                                        ...
                                        <button
                                            onClick={() => setIsAddingTraining(!isAddingTraining)}
                                            className="text-xs font-bold text-orange-400 hover:text-orange-300 flex items-center gap-1 bg-orange-500/10 px-3 py-1.5 rounded-lg">
                                            {isAddingTraining ? <X className="w-3.5 h-3.5" /> : <Plus className="w-3.5 h-3.5" />}
                                            {isAddingTraining ? 'CANCELAR' : 'REGISTRAR NOVO'}
                                        </button>
                                    </div>

                                    {isAddingTraining && (
                                        <div className="bg-orange-500/5 border border-orange-500/10 rounded-2xl p-6 mb-6 animate-in fade-in slide-in-from-top-2 duration-300">
                                            <div className="space-y-4">
                                                <div className="grid grid-cols-2 gap-4">
                                                    <div className="space-y-2">
                                                        <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Título do Treinamento</label>
                                                        <input
                                                            type="text"
                                                            value={newTrainingTitle}
                                                            onChange={(e) => setNewTrainingTitle(e.target.value)}
                                                            placeholder="Ex: Playbook de Abordagem"
                                                            className="w-full bg-black/40 border border-white/10 rounded-xl px-4 py-2 text-sm text-white font-bold outline-none focus:border-orange-500 transition-all"
                                                        />
                                                    </div>
                                                    <div className="space-y-2">
                                                        ...
                                                        <select
                                                            ...
                                                            className="w-full bg-black/40 border border-white/10 rounded-xl px-4 py-2 text-sm text-white font-bold outline-none focus:border-orange-500 transition-all appearance-none">
                                                            <option>Geral</option>
                                                            <option>Script de Vendas</option>
                                                            <option>Treinamento Técnico</option>
                                                            <option>Cultura & Onboarding</option>
                                                        </select>
                                                    </div>
                                                </div>
                                                <div className="space-y-2">
                                                    <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Observações / Detalhes</label>
                                                    <textarea
                                                        value={newTrainingDesc}
                                                        onChange={(e) => setNewTrainingDesc(e.target.value)}
                                                        placeholder="Descreva o que foi abordado e o resultado..."
                                                        className="w-full h-24 bg-black/40 border border-white/10 rounded-xl px-4 py-3 text-sm text-slate-300 font-medium outline-none focus:border-orange-500 transition-all resize-none"
                                                    />
                                                </div>
                                                <button
                                                    onClick={async () => {
                                                        if (!newTrainingTitle) return toast.error('Título é obrigatório');
                                                        await handleRegisterTraining(newTrainingTitle, newTrainingDesc, newTrainingCategory);
                                                        setNewTrainingTitle('');
                                                        setNewTrainingDesc('');
                                                        setIsAddingTraining(false);
                                                    }}
                                                    className="w-full py-3 bg-orange-500 text-black text-[10px] font-black uppercase tracking-widest rounded-xl hover:bg-orange-400 transition-all shadow-lg shadow-orange-500/20">
                                                    Confirmar Registro
                                                </button>
                                            </div>
                                        </div>
                                    ...
                                    <div className="space-y-4">
                                        {sellerTrainings.length === 0 && (
                                            <p className="text-center text-slate-600 text-xs py-10 uppercase tracking-widest font-black">Nenhum treinamento registrado.</p>
                                        )}
                                        {sellerTrainings.map((training: any) => (
